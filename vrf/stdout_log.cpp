...
std::shared_ptr<Logger> NewDefaultLogger()
{
    ...
    log_handlers[static_cast<std::size_t>(trace)] = [](const std::string &msg) { std::cout << "[trace] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(debug)] = [](const std::string &msg) { std::cout << "[debug] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(info)] = [](const std::string &msg) { std::cout << "[info] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(warning)] = [](const std::string &msg) { std::cerr << "[warning] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(error)] = [](const std::string &msg) { std::cerr << "[error] " << msg << '\n'; };
